...
public class GameSave_v2 : IGetOBJ<ILine>
{
    ...
    protected void Load(ILPS lps, Statistics? oldStatistics = null, GameSave_VPet? oldGameSave = null, ILPS? olddata = null)
    {
        ...
        {
            Statistics = new Statistics([.. lps["statistics"]]);
        }
        var vpet = lps.FindLine("vpet");
        ...
        if (vpet != null)
        {
            ...
            if (vpet.Remove("hash"))
            {
                ...
                try
                {
                    long hs = BitConverter.ToInt64(MD5.HashData(Encoding.UTF8.GetBytes(vpet.Name)), 0)
                        * 2 + BitConverter.ToInt64(MD5.HashData(Encoding.UTF8.GetBytes(vpet.info)), 0)
                        * 3 + BitConverter.ToInt64(MD5.HashData(Encoding.UTF8.GetBytes(vpet.text)), 0) * 4;
                    foreach (ISub su in vpet.ToList())
                    {
                        hs += BitConverter.ToInt64(MD5.HashData(Encoding.UTF8.GetBytes(su.Name)), 0) * 2
                            + BitConverter.ToInt64(MD5.HashData(Encoding.UTF8.GetBytes(su.Info)), 0) * 3;
                    }
                    HashCheck = hs == hash;
                }
                catch
