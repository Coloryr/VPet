...
public partial class PNGAnimation : IImageRun
{
    ...
    public PNGAnimation(GraphCore graphCore, string path, FileInfo[] paths, GraphInfo graphinfo, bool isLoop = false)
    {
        ...
        if (!GraphCore.CommConfig.ContainsKey("PA_Setup"))
        {
            ...
            GraphCore.CommUIElements["Image1.PNGAnimation"] = new Image() { Height = 500 };
            GraphCore.CommUIElements["Image2.PNGAnimation"] = new Image() { Height = 500 };
            GraphCore.CommUIElements["Image3.PNGAnimation"] = new Image() { Height = 500 }; // 多整个, 防止动画闪烁
        }
    ...
    private void Startup(string path, FileInfo[] paths)
    {
        ...
        try
        {
            ...
            if (!File.Exists(Path) && !((List<string>)GraphCore.CommConfig["Cache"]).Contains(path))
            {
                ...
                var img = SKBitmap.Decode(firstImage.FullName);
                w = img.Width;
                h = img.Height;
                ...
                using var joinedBitmap = new SKBitmap((int)w * paths.Length, (int)h, SKColorType.Rgba8888, SKAlphaType.Unpremul);
                using var graph = new SKCanvas(joinedBitmap);
                graph.DrawBitmap(img, new SKRect(0, 0, w, h));
                Parallel.For(1, paths.Length, i =>
                {
                    using var img = SKBitmap.Decode(paths[i].FullName);
                    lock (graph)
                        graph.DrawBitmap(img, new SKRect(w * i, 0, w, h));
                });
                if (!File.Exists(Path))
                {
                    using var data = joinedBitmap.Encode(SKEncodedImageFormat.Png, 100);
    ...
    public class Animation
    {
        ...
        {
            //先显示该图层
            Dispatcher.UIThread.Invoke(() => This.Margin = new Thickness(MarginWIX, 0, 0, 0));
            //然后等待帧时间毫秒
            Thread.Sleep(Time);
            //判断是否要下一步
            switch (Control.Type)
    ...
    public void Run(Decorator parant, Action EndAction = null)
    {
        ...
        Dispatcher.UIThread.Invoke(() =>
        {
            if (parant.Tag == this)
            {
                Task.Run(() => Animations[0].Run((Image)parant.Child, NEWControl));
                return;
            }
            Image img;
            ...
            {
                img = (Image)GraphCore.CommUIElements["Image1.PNGAnimation"];
            }
            else if (parant.Child == GraphCore.CommUIElements["Image3.PNGAnimation"])
            {
                img = (Image)GraphCore.CommUIElements["Image3.PNGAnimation"];
            }
            else
            {
                img = (Image)GraphCore.CommUIElements["Image2.PNGAnimation"];
