...
public class GraphInfo
{
    ...
    public GraphInfo(FileSystemInfo path, ILine info)
    {
        ...
        if (path is DirectoryInfo)
            ...
            pn = Sub.Split(path.FullName[..^path.Extension.Length].ToLower(), info[(gstr)"startuppath"]!.ToLower()).Last();
        ...
        if (!Enum.TryParse(info[(gstr)"graph"], true, out GraphType graphtype))
        {
            ...
            for (int i = 0; i < GraphHelper.GraphTypeValue.Length; i++)
            {//挨个找第一个匹配的
                if (path_name.Contains(GraphHelper.GraphTypeValue[i][0]))
                {
                    int index = path_name.IndexOf(GraphHelper.GraphTypeValue[i][0]);
                    bool ismatch = true;
                    for (int b = 1; b < GraphHelper.GraphTypeValue[i].Length && b + index < path_name.Count; b++)
                    {
                        if (path_name[index + b] != GraphHelper.GraphTypeValue[i][b])
                        {
                            ismatch = false;
                            break;
                        }
                    ...
                    if (ismatch)
                    {
                        ...
                        path_name.RemoveRange(index, GraphHelper.GraphTypeValue[i].Length);
                        break;
                    }
                }
            }
